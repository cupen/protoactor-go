...
func (pm *PartitionManager) onClusterTopology(tplg *ClusterTopologyEventV2) {
	...
	funclog := plog.With(log.Uint64("eventId", tplg.EventId))
	funclog.Debug("onClusterTopology start")
	...
	for kind, msg := range kindGroups {
		if v, ok := pm.kinds.Load(kind); ok {
			...
			if err := pk.start(chash); err != nil {
				funclog.Error("Start PartitionKind failed", log.String("kind", kind), log.Error(err))
			} else {
	...
	pm.kinds.Range(func(k, v interface{}) bool {
		...
		if _, ok := kindGroups[kind]; !ok {
			...
			pk.stop()
			funclog.Info("Stopped PartitionKind", log.String("kind", kind))
		}
		return true
	})
	funclog.Debug("onClusterTopology end")
}
