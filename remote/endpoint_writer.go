...
func (state *endpointWriter) initialize() {
	...
	if err != nil {
		plog.Error("EndpointWriter connect failed", log.String("address", state.address), log.Error(err))
...
func (state *endpointWriter) Receive(ctx actor.Context) {
	...
	case *actor.Stopped:
		...
		plog.Info("Stopped EnpointWriter", log.String("address", state.address))
